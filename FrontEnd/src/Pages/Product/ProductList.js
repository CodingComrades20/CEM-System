...
//                 <td><img src={imageData} width="100" height="100" /></td>
//                 <td>{product.name}</td>
        ...
        <table className="table border shadow">
          ...
          <tbody>
            {products.map((product, index) => (
              <tr key={index}>
                <th scope="row">{index + 1}</th>
                <td><img src={`http://localhost:8080/api/images/${product.id}`} width="100" height="100" style={{ borderRadius: '50%',border: '2px solid black' }}  /></td>
                <td>{product.name}</td>
                <td>{product.category}</td>
                <td>{product.quantity}</td>
                <td>{product.price}</td>
